...
if (empty($des)) $des = 'Tanımsız sınıflandırma';
...
<div id="header_info">
    <nav class="header-nav reveal">
        ...
        <div class="btn-group" style="float: right;margin-top: -65px;">
            <button type="button" class="btn btn-primary"><a href="<?php echo site_url() ?>" style="text-decoration:none;color:white"><i class="czs-hand-slide" style="margin-right:5px"></i> Ana Sayfaya geri dön</a></button>
        </div>
    </nav>
            ...
            <div class="link-list-right" :style="(post.post_categories[0].term_id == <?php echo $f; ?> || post.post_categories[0].term_id == <?php echo $w; ?> || post.post_categories[0].term_id == <?php tony_func('echo_s_cate'); ?>) ? '' : 'margin-top: -10px;'">
                ...
                <?php if($a){ ?>
                 ...
                 <?php } ?>
                    ...
                    <span class="article-list-minutes" v-if="post.post_categories[0].term_id !== <?php echo $f; ?> && post.post_categories[0].term_id !== <?php echo $w; ?>">{{ post.post_metas.views }}&nbsp;Görüntülendi</span>
                </div>
            </div>
        </template>
